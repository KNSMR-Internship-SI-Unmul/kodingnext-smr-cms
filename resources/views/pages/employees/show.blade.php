...
<div x-data="employeeManager({
        ...
        oldEmployeeId: @js(old('employee_id', $employee->id)),
        oldName: @js(old('name', $employee->name)),
        oldEmail: @js(old('email', $employee->email)),
        oldPhoneNumber: @js(old('phone_number', $employee->phone_number)),
        oldProfilePicture: @js(old('existing_image', $employee->profile_picture)),
        oldHiredDate: @js(old('hired_date', $employee->hired_date ? $employee->hired_date->format('Y-m-d') : '')),
        oldRoleId: @js(old('role_id', $employee->role_id)),
        oldImage: @js(old('existing_image', $employee->profile_picture))
    })" 
    ...
    <div class="flex justify-between items-end mb-8">
        ...
        <div class="flex items-center gap-4">
            ...
            @can('update', $employee)
            <button @click="openEditModal({{ json_encode($employee) }})" class="flex items-center justify-center px-6 py-2.5 w-[180px] bg-brand-pink hover:bg-brand-pink-hover text-white font-semibold rounded-lg transition text-sm gap-2">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-full">
            ...
            <div class="p-6 flex flex-col items-center flex-1">
                ...
                <div class="w-full mt-auto pt-6 border-t border-gray-100">
                    <button @click="openDeleteModal({{ $employee->id }})" class="w-full flex items-center justify-center gap-2 bg-[#FFF0F0] hover:bg-red-100 text-red-600 font-semibold py-3 rounded-xl transition-colors">
