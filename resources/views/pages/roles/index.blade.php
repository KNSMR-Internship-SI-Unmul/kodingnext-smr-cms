...
@section('content')
<div x-data="{ showRoleModal: {{ $errors->any() ? 'true' : 'false' }}, showDeleteModal: false }">
                    ...
                    <tr class="border-b border-gray-50 hover:bg-brand-light-blue-active transition-colors bg-brand-light-blue-active/75">
                        ...
                        <td class="py-3 px-4">
                            <div class="flex items-center gap-3">
                                {{-- Badge Inisial --}}
                                <div class="w-6 h-6 rounded bg-brand-light-pink text-brand-pink font-bold flex items-center justify-center text-sm shadow-sm">
                                    T
                                </div>
                                <span class="font-bold text-gray-900 text-sm">Teacher</span>
                            </div>
                        </td>
                        <td class="py-3 px-4 text-center font-medium text-gray-500 text-sm">14 Mar 2026</td>
                        <td class="py-3 px-4">
                            <div class="flex items-center justify-center gap-3">
                                <button @click="showRoleModal = true" class="text-brand-pink hover:text-brand-pink-hover transition-colors" title="Edit">
                    ...
                    <tr class="border-b border-gray-50 hover:bg-brand-light-blue-active transition-colors bg-brand-light-blue-active/75">
                        ...
                        <td class="py-3 px-4">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="font-bold text-gray-900 text-sm">Student Advisor</span>
                            </div>
                        </td>
                        <td class="py-3 px-4 text-center font-medium text-gray-500 text-sm">12 Mar 2026</td>
                        <td class="py-3 px-4">
                            <div class="flex items-center justify-center gap-3">
                                <button @click="showRoleModal = true" class="text-brand-pink hover:text-brand-pink-hover transition-colors" title="Edit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                    ...
                    <tr class="border-b border-gray-50 hover:bg-brand-light-blue-active transition-colors bg-brand-light-blue-active/75">
                        ...
                        <td class="py-3 px-4">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="font-bold text-gray-900 text-sm">Advance Teacher</span>
                            </div>
                        </td>
                        <td class="py-3 px-4 text-center font-medium text-gray-500 text-sm">10 Mar 2026</td>
                        <td class="py-3 px-4">
                            <div class="flex items-center justify-center gap-3">
                                <button @click="showRoleModal = true" class="text-brand-pink hover:text-brand-pink-hover transition-colors" title="Edit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
    ...
    <div x-show="showRoleModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 backdrop-blur-sm" x-transition.opacity>
        <div @click.away="showRoleModal = false" class="bg-white rounded-lg p-8 w-full max-w-lg shadow-2xl relative overflow-hidden" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-8" x-transition:enter-end="opacity-100 translate-y-0">
            
            <button @click="showRoleModal = false" class="absolute top-6 right-6 text-gray-400 hover:text-gray-700">
            ...
            <form action="#" method="POST" class="space-y-5">
                @csrf
                <div>
                    <label class="block text-sm font-semibold mb-2 text-gray-800">Role Name</label>
                    <input type="text" name="name" class="w-full px-4 py-2.5 text-sm rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-brand-pink transition" placeholder="e.g. Teacher">
                </div>

                <div class="flex gap-2 pt-2">
                    <button type="button" @click="showRoleModal = false" class="flex-1 py-2.5 bg-[#EE5B5B] hover:bg-red-600 text-white font-semibold rounded-lg transition text-sm">Cancel</button>
